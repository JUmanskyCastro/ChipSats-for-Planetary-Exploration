...
mass = zeros(n,1); area = zeros(n,1); Bcoeff = zeros(n,1)
Tmax = zeros(n,1); TimeToGround = zeros(n,1);
...
for i = 1:n
    ...
    p.Area_surf = 2*h*w;
    p.Area = area(i);
    q = 2*pi/n*i; %360 deployment
    mass(i) = normrnd(3/1000,0.1/1000);                 % mass of spacecraft, kg 
    %mass(i) = 3/1000; %kg, 5.8250e-06 for justin's 1cmx1cm chipsat
    p.m = mass(i);
    Bcoeff(i) = mass(i)/(p.Cd_FM*area(i)*10^6);
    %initial_position = [normrnd(initial_alt,0.1/1000), 0, 0];     % km, randomizing height of chipsat in deployer (10cm std dev)
    initial_position = [initial_alt, 0, 0];     % km, randomizing height of chipsat in deployer (10cm std dev)
...
figure;
yyaxis left
scatter(Bcoeff,Tmax,'b','filled')
xlabel('Ballistic coefficient kg/m^2')
ylabel('Temperature (C)')
hold on
yyaxis right
scatter(Bcoeff,TimeToGround/3600)
ylabel('Deorbit Time (min)')
