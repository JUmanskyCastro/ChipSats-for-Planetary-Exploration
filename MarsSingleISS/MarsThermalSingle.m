...
for i = 1:n
    ...
    %area(i) = h*w*normrnd(0.6,0.3);  % km^2, tumbling average area (1 = worse case, face on)
    area(i) = h*w;
...
legend('Mach number','Temperature','Location','NorthWest')
...
%      mu = polyval(visc,z); % dynamic viscosity (N s/m^2)
    ...
    u = [Ma, ST, Kn, T_CO2,alt,Re,Re2,Re2*Kn2,Cd,Vs(1),Vs(2),Vs(3),norm(drag)];
end 
